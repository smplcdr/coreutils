...
static volatile int digits = 2;

/* Number of files created so far.  */
static volatile unsigned int files_created = 0;
...
static volatile bool remove_files;
...
/* Return the name of output file number NUM.
   ...
   not guarantee this for the functions called below, but we do not
   know of any hosts where this implementation is not safe.  */
...
static struct control *
extract_regexp (int argnum, bool ignore, const char *str)
{
  size_t len;     /* Number of bytes in this regexp.  */
  char delim = *str;
  const char *closing_delim;
  struct control *p;
  const char *err;
...
static size_t
get_format_flags (const char *format, int *flags_ptr)
{
  int flags = 0;
...
main (int argc, char **argv)
{
  ...
  while ((optc = getopt_long (argc, argv, "f:b:kn:sqz", long_options, NULL)) != -1)
    switch (optc)
      {
      case 'f':
        prefix = optarg;
