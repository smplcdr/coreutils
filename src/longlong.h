...
/* count_leading_zeros_gcc_clz is count_leading_zeros implemented with gcc
   ...
   it is not used (for count_leading_zeros) because it generally gives extra
   code to ensure the result is 0 when the input is 0, which we do not need
   or want.  */
...
/* FIXME: The macros using external routines like __MPN(count_leading_zeros)
   do not need to be under !NO_ASM */
#if ! defined (NO_ASM)
...
/* similar to what gcc does for __builtin_ffs, but 0 based rather than 1
   based, and we do not need a special case for x==0 here */
#define count_trailing_zeros(count, x)          \
  do {                  \
    UWtype __ctz_x = (x);           \
    __asm__ ("popcnt %0 = %1"           \
...
/* These macros are for ABI=2.0w.  In ABI=2.0n they cannot be used, since GCC
...
/* On x86 and x86_64, every asm implicitly clobbers "flags" and "fpsr",
   so we do not need __CLOBBER_CC.  */
...
/* The following should be a fixed 14 or 15 cycles, but possibly plus an L1
   cache miss reading from __clz_tab.  For P55 it is favoured over the float
...
/* The following should be a fixed 14 cycles or so.  Some scheduling
   ...
   apparently suggested by the Intel optimizing manual (do not know exactly
...
/* Do not match immediate range because, 1) it is not often useful,
...
#else
/* Do not use this on SuperSPARC because its udiv only handles 53 bit
...
" sra %3,31,%%g2  ! Do not move this insn\n"    \
" and %2,%%g2,%%g2  ! Do not move this insn\n"    \
...
/* If we still do not have umul_ppmm, define it using plain C.
...
/* If we do not have smul_ppmm, define it using umul_ppmm (which surely will
